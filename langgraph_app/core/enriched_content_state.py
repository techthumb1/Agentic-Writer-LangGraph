...
class EnrichedContentState:
    ...
    def get_writing_context(self) -> Dict[str, Any]:
        """Extract simple writing context for Writer agent"""

        # Basic required fields - fail only if completely missing
        if not self.content_spec:
            raise ValueError("content_spec is required for content generation")

        topic = getattr(self.content_spec, 'topic', '').strip()
        if not topic:
            raise ValueError("content_spec.topic is required for content generation")

        # Extract context with defaults
        context = {
            'topic': topic,
            'audience': getattr(self.content_spec, 'target_audience', 'general audience'),
            'platform': getattr(self.content_spec, 'platform', 'web'),
            'template_type': getattr(self.content_spec, 'template_type', 'article')
        }
    
        return context

    def get_style_prompt(self) -> str:
        """Get style prompt from style_config - return default if missing"""

        if not self.style_config or not isinstance(self.style_config, dict):
            return "Write in a professional, engaging style appropriate for the target audience."

        # Try to get system_prompt, fall back to building from style elements
        system_prompt = self.style_config.get('system_prompt', '')

        if system_prompt and system_prompt.strip():
            return system_prompt.strip()

        # Build basic style guidance from available style_config
        tone = self.style_config.get('tone', 'professional')
        voice = self.style_config.get('voice', 'authoritative')
        formality = self.style_config.get('formality', 'balanced')

        return f"Write in a {tone} tone with {voice} voice and {formality} formality level."

    def get_planning_guidance(self) -> str:
        """Get planning guidance from planning_output - return empty if missing"""

        if not hasattr(self, 'planning_output') or not self.planning_output:
            return ""

        guidance_parts = []

        # Extract structure approach
        if hasattr(self.planning_output, 'structure_approach'):
            structure = getattr(self.planning_output, 'structure_approach', '')
            if structure and structure.strip():
                guidance_parts.append(f"Structure: {structure}")

        # Extract key messages
        if hasattr(self.planning_output, 'key_messages'):
            messages = getattr(self.planning_output, 'key_messages', [])
            if messages and isinstance(messages, list):
                guidance_parts.append(f"Key Messages: {', '.join(messages)}")

        # Extract content strategy
        if hasattr(self.planning_output, 'content_strategy'):
            strategy = getattr(self.planning_output, 'content_strategy', '')
            if strategy and strategy.strip():
                guidance_parts.append(f"Strategy: {strategy}")
    ...
    def has_required_content_inputs(self) -> bool:
        """Check if minimum inputs for content generation are available"""
        return (
            self.content_spec is not None and 
            hasattr(self.content_spec, 'topic') and 
            bool(getattr(self.content_spec, 'topic', '').strip())
        )
    
    def log_missing_configs(self) -> None:
        """Log missing configs for debugging without blocking execution"""
        import logging
        logger = logging.getLogger(__name__)
        
        if not self.template_config:
            logger.warning("template_config is empty - using defaults")
        
        if not self.style_config:
            logger.warning("style_config is empty - using defaults")
        
        if not self.has_required_content_inputs():
            logger.warning("content_spec missing required topic")
@dataclass
