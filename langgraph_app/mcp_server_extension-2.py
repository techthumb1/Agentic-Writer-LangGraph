...
from langgraph_app.core.enriched_content_state import ContentSpec, ContentType, GenerationStatus
...
class ConsolidatedMCPOrchestrator:
    ...
    def _initialize_universal_system(self):
        ...
        try:
            ...
            logger.warning(f"Universal System not available: {e}")
            self.universal_integration = None
    ...
    async def _execute_mcp_tools(
        ...
        tools_cfg = mcp_config.get("tools", {}) or {}
        if not tools_cfg:
            return {"tools_executed": [], "research_data": {}, "quality_enhancements": {}}
    
        tools_executed = []
        research_data = {}
        quality_enhancements = {}
    
        max_concurrency = int(mcp_config.get("concurrency", 5))
        timeout_sec = int(mcp_config.get("tool_timeout_sec", 20))
        sem = asyncio.Semaphore(max_concurrency)
    
        async def run_tool(tname: str, tcfg: Dict[str, Any]):
            ...
            async with sem:
                try:
                    ...
                    if res.get("success"):
                        if "research" in tname or "search" in tname:
                            research_data[tname] = res.get("result", {})
                        else:
                            quality_enhancements[tname] = res.get("result", {})
                        tools_executed.append(tname)
                        logger.info(f"✅ MCP tool {tname} executed")
                except Exception as e:
                    logger.error(f"❌ MCP tool {tname} failed: {e}")
    
        await asyncio.gather(*(run_tool(name, cfg) for name, cfg in tools_cfg.items()))
...
# File: langgraph_app/mcp_server_extension.py
# Lines 345-400 - REPLACE the build_initial_state method
...
class MCPGenerationRequest(BaseModel):
    ...
    def build_initial_state(
        ...
        """
        ENTERPRISE FIXED: Build initial state as EnrichedContentState object
        Returns actual state object instead of dict to prevent config loss
        """
        ...
        if not isinstance(dynamic_parameters, dict):
            raise ValueError("ENTERPRISE: dynamic_parameters must be a dict")

        # Extract from dynamic_overrides (actual user input)
        user_overrides = dynamic_parameters.get("dynamic_overrides", {})
        if not user_overrides:
            raise ValueError("ENTERPRISE: Missing dynamic_overrides with user input")
        
        # Extract actual user topic - fail fast if missing
        topic = user_overrides.get("topic")
        if not topic or not topic.strip():
            raise ValueError(f"ENTERPRISE: Missing or empty topic in dynamic_overrides: {user_overrides}")
        
        # Use actual template type from config
        template_type = template_config.get("template_type")
        if not template_type:
            template_type = template_config.get("id", template_config.get("slug", "article"))
        
        # Extract audience from user input
        audience = (
            user_overrides.get("target_audience") or 
            user_overrides.get("audience") or
            style_config.get("target_audience") or
            "general"
        )
        
        platform = user_overrides.get("platform", "web")
        
        logger.info(f"ENTERPRISE STATE: topic='{topic}', type='{template_type}', audience='{audience}'")
        
        # Create complete ContentSpec with all required fields
        content_spec = ContentSpec(
            ...
            length=template_config.get("defaults", {}).get("length", 1000),
            template_id=template_config.get("id", "default"),
            style_profile=style_config.get("id", "default"),
            complexity_level=5
        )
        
        # CRITICAL FIX: Create EnrichedContentState object directly
        ...
        # Store dynamic parameters and metadata
        state.generation_settings = copy.deepcopy(dynamic_parameters)
        state.debug_info = {
            "enterprise_mode": True,
            "mcp_enabled": True,
            "actual_topic_used": topic,
            "template_type_used": template_type,
            "state_created_at": datetime.utcnow().isoformat()
        }
        
        return state

    @staticmethod
    def build_mcp_options(request_id: str, enable_mcp: bool = True) -> Dict[str, Any]:
        return {
            "request_id": request_id,
...
async def execute_enhanced_mcp_generation(
    ...
    """
    FIXED: Enterprise MCP generation using MCPGenerationRequest.build_initial_state
    """
    from langgraph_app.mcp_enhanced_graph import MCPEnhancedContentGraph
    ...
    try:
        # FIXED: Use the class method instead of standalone function
        initial_state = MCPGenerationRequest.build_initial_state(
            request_id, template_config, style_config, dynamic_parameters
        )
        
        logger.info(f"DEBUG: Built initial state with keys: {list(initial_state.keys())}")
        
        # Initialize graph and execute
        graph = MCPEnhancedContentGraph()
        mcp_opts = MCPGenerationRequest.build_mcp_options(request_id, enable_mcp=True)
        
        result = await graph.execute_coordinated_generation(initial_state, mcp_options=mcp_opts)
...
# FIXED: Add the missing import and ensure endpoints are properly accessed
def add_mcp_endpoints(app: FastAPI) -> None:
    """Add MCP endpoints to FastAPI app - FIXED to be accessible"""
    
    @app.get("/api/mcp/health")
    async def mcp_health_check():
        """MCP health check endpoint"""
        try:
            # FIXED: Check if enhanced_mcp_manager exists and is initialized
            if not hasattr(globals(), 'enhanced_mcp_manager') or not enhanced_mcp_manager:
                return JSONResponse(
                    content={"status": "unavailable", "message": "MCP manager not available"},
                    status_code=503
                )
    ...
    # FIXED: Add endpoint registration logging
    logger.info("MCP endpoints registered: /api/mcp/health, /api/mcp/tools")

# FIXED: Update the initialize_mcp_for_existing_server function to ensure endpoints are accessible
async def initialize_mcp_for_existing_server(app: FastAPI) -> bool:
    """Initialize MCP for existing server - FIXED to ensure endpoint access"""
    try:
        logger.info("Initializing Consolidated MCP...")
        
        # Ensure the enhanced_mcp_manager is available globally
        global enhanced_mcp_manager
        if not enhanced_mcp_manager:
            from . import enhanced_mcp_manager  # Import if not available
        
        enhanced_mcp_manager.app_state = app.state
        await enhanced_mcp_manager.initialize_servers()
        
        # FIXED: Register endpoints after manager is initialized
        ...
        logger.info(f"MCP endpoints available at: /api/mcp/health, /api/mcp/tools")
        return True
...
graph = MCPEnhancedContentGraph().graph
