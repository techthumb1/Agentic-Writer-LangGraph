...
def safe_get_template(template_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        return style_loader.get_profile(template_id)
...
def safe_get_style_profile(profile_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        return style_loader.get_profile(profile_id)
    except Exception as e:
...
def get_all_templates_sync() -> List[Dict[str, Any]]:
    ...
    templates = []
    template_profiles = [
        "startup_trends_brief", "technical_dive", "founder_storytelling", 
        "beginner_friendly", "educational_expert", "future_of_llms",
        "ai_in_healthcare", "experimental_lab_log", "startup_usecases"
    ]
    
    for profile_id in template_profiles:
        profile_data = style_loader.get_profile(profile_id)
        if profile_data:
            template_info = {
                "id": profile_id,
                "name": profile_data.get("name", profile_id.replace("_", " ").title()),
                "description": profile_data.get("description", f"Template for {profile_id}"),
...
def get_all_style_profiles_sync() -> List[Dict[str, Any]]:
    ...
    profiles = []
    for profile_id, profile_data in style_loader.profiles_cache.items():
        profile_info = {
            "id": profile_id,
            "name": profile_data.get("name", profile_id.replace("_", " ").title()),
