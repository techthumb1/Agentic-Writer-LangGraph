...
class TemplateAwareWriterAgent(RealTimeSearchMixin):
    ...
    def _supports_temperature(self, model_name: str) -> bool:
        ...
        for non_temp_model in non_temperature_models:
            if model_lower.startswith(non_temp_model.lower()):
                return False
        return True

    def _call_openai(self, model_name, system_content, user_content, max_tokens, temperature, generation_settings):
        """
        Call OpenAI API with circuit breaker protection and intelligent temperature handling.
        
        Improvements:
        - Circuit breaker integration (prevents hammering overloaded APIs)
        ...
        if not circuit_breaker.can_execute(provider):
            raise RuntimeError(
                "OpenAI API circuit breaker is OPEN due to repeated failures. "
                "The service may be experiencing issues. Please try again in a few minutes."
            )
        ...
        if supports_temperature:
            ...
            logger.info(f"Model {model_name} does not support custom temperature, using default")
    
        # Retry configuration
        max_attempts = 3
        delays = [2.0, 5.0, 10.0]  # Exponential-ish backoff
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt > 0:
                    logger.info(f"✅ Writer API call succeeded on retry {attempt + 1}/{max_attempts}")
                ...
                # Special handling for temperature parameter error
                if "temperature" in error_str and "unsupported" in error_str:
                    logger.warning(f"Temperature not supported for {model_name}, retrying without temperature parameter")
                    api_kwargs.pop("temperature", None)
                    continue  # Immediate retry without recording failure
                ...
                if attempt < max_attempts - 1 and is_retryable:
                    ...
                    logger.warning(
                        f"⚠️ Writer API call failed with {error_type} "
                    ...
                    logger.error(
                        f"❌ Writer API call failed: {error_type} - {str(e)}. "
                        f"Attempt {attempt + 1}/{max_attempts}."
                    )
                    raise
                
        # Should never reach here, but for safety
        raise RuntimeError(f"Writer API call failed after {max_attempts} attempts")
    ...
    def _extract_user_inputs(self, state: EnrichedContentState) -> Dict:
        ...
        return inputs
    def extract_context_values(self, state: EnrichedContentState) -> Dict:
        context = {
    ...
    def analyze_context(self, state: EnrichedContentState) -> WritingContext:
        params = state.dynamic_parameters or {}
        raw_template_config = state.template_config or {}
    ...
    def has_mcp_research(self, state: EnrichedContentState) -> bool:
        research_indicators = [
            state.research_findings,
            state.mcp_results if hasattr(state, 'mcp_results') else None,
            state.tools_executed if hasattr(state, 'tools_executed') else None,
            state.research_data if hasattr(state, 'research_data') else None,
            state.quality_enhancements if hasattr(state, 'quality_enhancements') else None
        ]
    ...
    def extract_mcp_research_data(self, state: Dict) -> Dict[str, Any]:
        ...
        if getattr(state, 'mcp_results', None):
        ...
        if getattr(state, 'tools_executed', None):
    ...
    def _generate_adaptive_content(self, state: Dict[str, Any]) -> str:
        ...
        try:
            template_id = getattr(state, "template", "")
            ...
            response = self._call_openai(
                ...
                temperature=generation_settings.get("temperature", 1.0),
    ...
    def _extract_content_from_openai_response(self, response) -> str:
        try:
            logger.info("DEBUG: Starting content extraction...")
            ...
            # Method 1: Standard completion response
            if hasattr(response, 'choices') and response.choices:
                logger.info(f"DEBUG: Found choices, length: {len(response.choices)}")

                # Check finish_reason before extracting content
                if hasattr(response.choices[0], 'finish_reason'):
                    finish_reason = response.choices[0].finish_reason
                    logger.info(f"DEBUG: finish_reason = {finish_reason}")
                    if finish_reason == 'length':
                        raise RuntimeError(
                            "ENTERPRISE: Model hit token limit before generating content. "
                            "Increase max_tokens in settings or reduce prompt complexity."
                        )

                if hasattr(response.choices[0], 'message'):
                    logger.info(f"DEBUG: Message attributes: {dir(response.choices[0].message)}")
                    if hasattr(response.choices[0].message, 'content'):
                        content = response.choices[0].message.content
                        logger.info(f"DEBUG: Extracted from message.content, type: {type(content)}")
                    elif hasattr(response.choices[0].message, 'text'):
                        content = response.choices[0].message.text
                        logger.info(f"DEBUG: Extracted from message.text")
                elif hasattr(response.choices[0], 'text'):
                    content = response.choices[0].text
                    logger.info(f"DEBUG: Extracted from choices[0].text")

            # Method 2: Legacy output_text
            elif hasattr(response, 'output_text'):
                content = response.output_text
                logger.info(f"DEBUG: Extracted from output_text")

            # Method 3: Direct text attribute
            elif hasattr(response, 'text'):
                content = response.text
                logger.info(f"DEBUG: Extracted from text")

            # Method 4: Fallback to string conversion
            if content is None:
                logger.warning("DEBUG: No standard extraction method worked, attempting string conversion")
                logger.info(f"DEBUG: Full response object: {response}")
                content = str(response)
                logger.info(f"DEBUG: Converted to string")

            logger.info(f"DEBUG: Content type after extraction: {type(content)}")
            logger.info(f"DEBUG: Content value (first 200 chars): {str(content)[:200] if content else 'None'}")

            if content is None:
                raise RuntimeError("ENTERPRISE: API returned None content")

            if not isinstance(content, str):
                logger.warning(f"DEBUG: Converting non-string content from {type(content)} to string")
                content = str(content)

            # Don't strip if this removes actual content
            original_length = len(content)
            content = content.strip()
            logger.info(f"DEBUG: Content length - original: {original_length}, after strip: {len(content)}")

            if not content or len(content) == 0:
                logger.error(f"DEBUG: Empty content detected. Original type: {type(response)}")
                logger.error(f"DEBUG: Response has choices: {hasattr(response, 'choices')}")
                if hasattr(response, 'choices') and response.choices:
                    logger.error(f"DEBUG: First choice: {response.choices[0]}")
                    logger.error(f"DEBUG: First choice attributes: {dir(response.choices[0])}")
                raise RuntimeError("ENTERPRISE: API returned empty content after strip")

            logger.info(f"DEBUG: Content extraction successful, length: {len(content)}")
            return content

        except Exception as e:
            logger.error(f"ERROR: Content extraction failed: {e}", exc_info=True)
            raise RuntimeError(f"ENTERPRISE: Content extraction failed: {e}")
    
    def _get_user_generation_settings(self, state: EnrichedContentState) -> Dict[str, Any]:
        gen: Dict[str, Any] = {}
        
        dyn = state.dynamic_parameters or {}
        ...
        fallback_sources = [
            getattr(state, 'generation_settings', {}),
            getattr(state, 'user_settings', {}),
            getattr(state.content_spec, 'generation_settings', {}) if state.content_spec else {},
    ...
    def _sanitize_and_enforce(self, raw: str, template_config: Dict = None, state: Dict = None) -> str:
        ...
        template_id = str(template_config.get('id', template_config.get('slug', '')))

        if template_type == 'blog_article' or 'blog' in template_id.lower():
            # Remove YAML front matter
            text = re.sub(r"^---[\s\S]*?---\s*", "", text, flags=re.MULTILINE)

            # Remove explicit section labels
            forbidden_headings = [
                r"^\s*Hook\s*$",
                r"^\s*Conclusion\s*$",
                r"^\s*Conclusion and Call to Action\s*$",
                r"^\s*Call to Action\s*$",
                r"^\s*Frequently Asked Questions\s*$",
                r"^\s*FAQ[s]?\s*$"
            ]
            for pat in forbidden_headings:
                text = re.sub(pat, "", text, flags=re.MULTILINE | re.IGNORECASE)

            # Strip obvious CTA lines
            cta_patterns = [
                r"(?i)here['’]s your next step[:]?.*",
                r"(?i)book a [0-9]+-?minute .*call.*",
                r"(?i)want help .*",
    ...
    def _enforce_blog_narrative_style(self, text: str) -> str:
        ...
        heading_pattern = re.compile(r"^(#+\s*.*|[A-Z][A-Za-z ]{0,60}:)$")
    ...
    def _convert_bullets_to_narrative(self, bullet_text: str) -> str:
        ...
        if len(items) == 1:
            ...
            narrative = ", ".join(items[:-1]) + f", and {items[-1]}."
...
writer = RunnableLambda(template_aware_writer_agent.execute)
