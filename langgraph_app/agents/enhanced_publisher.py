...
class IntelligentPublisherAgent:
    """
    Intelligent Publisher Agent that orchestrates multi-platform publishing
    """
...
PublisherAgent = IntelligentPublisherAgent

# enhanced_publisher.py - Add to the very end:
from langchain_core.runnables import RunnableLambda

async def _publisher_fn(state: dict) -> dict:
    """Publisher agent function for LangGraph workflow"""
    publisher_agent = IntelligentPublisherAgent()
    return await publisher_agent.intelligent_publish(state)

publisher = RunnableLambda(_publisher_fn)
